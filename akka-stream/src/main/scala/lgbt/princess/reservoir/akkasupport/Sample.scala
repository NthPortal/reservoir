...
/**
 * Utility for randomly sampling elements from a stream, without keeping any elements other than those sampled in
...
object Sample {
  ...
  /**
   * Creates a stream operator that samples elements with equal probability, and allows duplicate elements in the
   * sample.
   *
   * @param maxSampleSize
   *   the maximum number of elements to sample
   * @param preAllocate
   ...
   *   if `map` is `null`
   * @return
   *   a stream operator that randomly samples elements of the stream
   */
  ...
  /**
   * Creates a stream operator that samples distinct values with equal probability; it does not allow duplicate elements
   * in the sample.
   *
   * @note
   ...
   * @return
   *   a stream operator that randomly samples distinct elements of the stream
   */
  @throws[IllegalArgumentException]
  @throws[NullPointerException]
